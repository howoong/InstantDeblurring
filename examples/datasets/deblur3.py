...
def get_bbox3d_for_llff(poses, H,W,focal, near=0.0, far=1.0):
    H, W = int(H), int(W)
...
def _load_renderings(root_fp: str, subject_id: str, split: str, hold_every:int, downsample:int):
    ...
    bbox = (torch.tensor(min_bound)-torch.tensor([0.1,0.1,0.0001]), torch.tensor(max_bound)+torch.tensor([0.1,0.1,0.0001]))

    return all_rgbs, all_rays, N, img_list, bbox
...
class SubjectLoader(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.rgbs, self.rays, self.N, img_list, self.aabb = _load_renderings(
        ...
        if num_rays != None:
            self.init_focus(data_dir, img_list, subject_id, fmo, tag, lof, grouping)
    ...
    def init_focus(self, data_dir=None, img_list=None, scene_name="defocustools", fmo="AIF", tag=None, lof=400, grouping="quantile"):
        if fmo == "AIF":
            focus_path = os.path.join(data_dir,f"focus_map_{fmo}_7.npy")   # norm 안된 것. 
            print("focus_map is not normalized!!!!!!!")
        elif fmo == "SML":
            focus_path = os.path.join(data_dir,f"focus_map_{fmo}_7.npy")
        else:
            raise NotImplementedError
        ...
        focus_map = torch.from_numpy(np.load(focus_path)) # (N, H, W) or (N, H, W, 3)
        if fmo == "AIF":
            focus_map = 1 - focus_map
        
        focus_map_train = focus_map[img_list]
        self.focus_map = focus_map_train
        focus_lv = torch.zeros_like(focus_map_train).to(torch.int64) # N_tr, H,W or N_tr,H,W,3
        ...
        if grouping == "quantile":
            ...
            split = math.ceil(self.HEIGHT*self.WIDTH // lof)
            for i in range(lof):
                if i == lof-1:     # for the case where H*W / self.lof does not produce integer value.
                    mask = idx[:, i*split : ]
                else:
                    mask = idx[:, i*split : (i+1)*split]
                for j in range(N_tr_img):
                    focus_lv[j, mask[j]] = i
            self.focus_lv = focus_lv.view(-1, self.HEIGHT, self.WIDTH)
        elif grouping == "depthQ":
            self.focus_lv = torch.from_numpy(np.load(f"/home/blee/nfs/SFF/psf/SharpNeRF/preprocess/DEPTHQ/{scene_name}_{tag}_sort_{fmo}.npy")).to(torch.int64)
            self.focus_lv = self.focus_lv[img_list]
            self.lof = self.focus_lv.unique().shape[0]
            print("Depth Q LOF: ", self.lof)
        elif grouping == "noimgwise":
            self.focus_lv = torch.from_numpy(np.load(f"/home/blee/nfs/SFF/psf/SharpNeRF/preprocess/NOIMWISE/{scene_name}_{tag}.npy")).to(torch.int64)
    ...
    def fetch_data(self, index):
        ...
        if self.training:
            ...
            for iid,pid in zip(image_id, patch_id):    # 매번 이걸 돌아야 한다는게.. 일단은 시간비교 해봐야함
                ...
                focus_lv_train.append(self.focus_lv[iid, h:hh, w:ww])

            rgb = torch.stack(rgb_train).reshape(-1,3).to(self.device)
            rays = torch.concat(rays_train).to(self.device)
            focus_lv = torch.stack(focus_lv_train).to(self.device)
